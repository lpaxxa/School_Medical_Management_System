...
const USE_MOCK_API = true; // Always use mock API

// API URL if needed (not used with mock data)
...
const userApiService = {
  ...
  resetUserPassword: async (userId, newPassword) => {
    if (USE_MOCK_API) {
      ...
      // Update user status (in real app, would hash password)
      mockUsers[userIndex].matKhau = "******"; // Password is hidden in response
      mockUsers[userIndex].ngayCapNhat = new Date().toISOString().split('T')[0];
    ...
    if (
      ...
      try {
        ...
        setUsers((prevUsers) =>
          prevUsers.map((user) =>
            user.id === userId
              ? { ...user, ngayCapNhat: new Date().toISOString().split("T")[0] }
              : user
          )
        );
      } catch (error) {
        console.error("Error resetting password:", error);
        alert(`Không thể đặt lại mật khẩu. Vui lòng thử lại.`);        // If we're in demo mode, still show the temp password
        if (error.message?.includes("demo") || !import.meta.env.VITE_API_URL) {
