...
import './AdminStudentImageUpload.css';
...
const AdminStudentImageUpload = ({ 
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await onImageDelete(student.id);
    } catch (error) {
  ...
  return (
    <>
      <div className="admin-student-image-upload">
          ...
          {student.imageUrl ? (
            <div className="admin-image-wrapper">
              ...
              {!disabled && (
                <div className="admin-image-overlay">
                  <button
                    type="button"
                    className="admin-image-action-btn admin-upload-btn"
                    onClick={() => document.getElementById(`admin-file-input-${student.id}`).click()}
                  ...
                  <button
                    type="button"
                    className="admin-image-action-btn admin-delete-btn"
                    onClick={handleDelete}
                    disabled={isUploading || isDeleting}
                    title="Xóa ảnh"
                  >
                    {isDeleting ? <FaSpinner className="admin-spinning" /> : <FaTrash />}
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="admin-no-image">
              ...
              {!disabled && (
                <button
                  type="button"
                  className="admin-upload-image-btn"
                  onClick={() => document.getElementById(`admin-file-input-${student.id}`).click()}
                  disabled={isUploading || isDeleting}
                >
                  <FaCamera />
        ...
        {!disabled && (
          <input
            id={`admin-file-input-${student.id}`}
            type="file"
      ...
      {/* Upload Modal */}
      {showModal && (
        <div className="admin-image-upload-modal-overlay">
          <div className="admin-image-upload-modal-dialog">
...
export default AdminStudentImageUpload;
