...
const receiveMedicineService = {
  ...
  getAllMedicineRequests: async () => {
    try {
      const response = await fetch(`${BASE_URL}/all-requests`);
      if (!response.ok) {
  ...
  // Xử lý yêu cầu thuốc (phê duyệt/từ chối) - Using real API
    ...
    try {
      const response = await fetch(`${BASE_URL}/${id}/process`, {
        ...
        body: JSON.stringify({
          ...
          reasonProvidedWhenRequired: true
        })
      });

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || 'Có lỗi xảy ra khi xử lý yêu cầu');
      }
